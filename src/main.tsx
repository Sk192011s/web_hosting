...
<script>
    ...
    async function startJobPolling(jobId) {
        ...
        jobCard.innerHTML = \`
            <div class="flex justify-between items-center mb-2">
                ...
                <span class="text-xs text-zinc-400" id="status-\${jobId}">Starting...</span>
            </div>
        ...
        const poll = setInterval(async () => {
            try {
                ...
                if (data.status === 'processing') {
                    statusEl.innerText = \`Downloading & Uploading (\${data.progress}%)\`;
    ...
    // Initialize existing active jobs
    ...
    async function confirmDelete() {
        ...
        setLoading('btnConfirmDelete', true, 'Deleting...');
        try {
    ...
    async function uploadLocal(event) {
        ...
        if(fileInput.files.length === 0) { showToast("Choose a file", 'error'); return; }
        setLoading('submitBtn', true, 'Checking...');
        ...
        try {
            ...
            xhr.onload = async () => {
                if (xhr.status === 200) {
                    ...
                    showToast('Upload Successful!'); setTimeout(() => window.location.reload(), 1000);
    ...
    async function uploadRemote(event) {
        ...
        if(!urlInput.value) { showToast("Enter URL", 'error'); return; }
        ...
        try {
            ...
            showToast("Job Started! Check progress below.");
            urlInput.value = ''; 
...
app.get("/", async (c) => {
    ...
    if (maintenance && session?.user.username !== ADMIN_USERNAME) return c.html(<Layout><div class="text-center mt-32 text-yellow-500"><i class="fa-solid fa-screwdriver-wrench text-4xl"></i><h1 class="text-2xl font-bold mt-4">Maintenance Mode</h1></div></Layout>);
    if(!session) return c.redirect("/login");
    const { user, csrfToken } = session;
    if(user.isBanned) return c.text("Account Banned", 403);
    ...
    return c.html(<Layout user={user} csrfToken={csrfToken} activeJobs={activeJobs}>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            <div class="glass p-5 rounded-2xl">
                <p class="text-xs text-zinc-500 font-bold uppercase">Plan</p>
            ...
            <div class="glass p-5 rounded-2xl">
                <div class="flex justify-between mb-1"><span class="text-xs font-bold text-zinc-500 uppercase">Storage</span><span class="text-white font-bold">{totalGB} / {limitGB} GB</span></div>
        ...
        <div class="glass p-6 rounded-2xl mb-8 border border-zinc-700/50">
            <div class="flex gap-4 mb-6 border-b border-zinc-800 pb-4">
                <button id="btn-mode-local" onclick="switchUploadMode('local')" class="mode-btn px-4 py-2 text-xs font-bold rounded-lg bg-yellow-500 text-black flex gap-2"><i class="fa-solid fa-upload"></i> Local File</button>
                <button id="btn-mode-remote" onclick="switchUploadMode('remote')" class="mode-btn px-4 py-2 text-xs font-bold rounded-lg bg-zinc-800 text-gray-400 flex gap-2"><i class="fa-solid fa-globe"></i> Remote URL</button>
            </div>

            <div id="mode-local" class="upload-mode">
                <form id="uploadForm" onsubmit="uploadLocal(event)" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input name="customName" placeholder="Filename (Optional)" class="bg-black border border-zinc-700 rounded-xl p-3 text-sm text-white w-full" />
                        {isVipActive(user) ? <select name="expiry" class="bg-black border border-yellow-600/50 rounded-xl p-3 text-sm text-yellow-500 w-full"><option value="0">Lifetime</option><option value="7">7 Days</option></select> : <input disabled value="30 Days (Free)" class="bg-zinc-900 border border-zinc-700 text-gray-500 rounded-xl p-3 text-sm w-full" />}
                    </div>
                    ...
                    <div class="border-2 border-dashed border-zinc-800 rounded-2xl p-6 text-center hover:bg-zinc-900 relative cursor-pointer">
                        ...
                        <i class="fa-solid fa-plus text-2xl text-zinc-600 mb-2"></i><p id="fileNameDisplay" class="text-sm font-bold text-zinc-400">Select File</p>
                    </div>
                    ...
                    <button id="submitBtn" class="w-full bg-yellow-500 text-black font-bold py-3 rounded-xl">Upload</button>
                </form>
            </div>
            ...
            <div id="mode-remote" class="upload-mode hidden">
                <form onsubmit="uploadRemote(event)" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input id="remoteName" placeholder="Filename (Optional)" class="bg-black border border-zinc-700 rounded-xl p-3 text-sm text-white w-full" />
                        <select name="expiry_remote" class="bg-black border border-yellow-600/50 rounded-xl p-3 text-sm text-yellow-500 w-full"><option value="0">Lifetime</option><option value="7">7 Days</option></select>
                    </div>
                    ...
                    <button id="remoteBtn" class="w-full bg-zinc-800 text-white font-bold py-3 rounded-xl hover:bg-zinc-700">Start Background Upload</button>
                </form>
            </div>
        </div>
        ...
        <div class="flex justify-between items-center mb-4">
            <h3 class="font-bold text-white uppercase text-sm">My Files</h3>
            <div class="flex gap-2">
                <a href="/?type=all" class={`px-3 py-1 text-[10px] font-bold rounded ${filterType==='all'?'bg-yellow-500 text-black':'bg-zinc-800 text-gray-400'}`}>ALL</a>
                <a href="/?type=video" class={`px-3 py-1 text-[10px] font-bold rounded ${filterType==='video'?'bg-yellow-500 text-black':'bg-zinc-800 text-gray-400'}`}>VID</a>
                <a href="/?type=image" class={`px-3 py-1 text-[10px] font-bold rounded ${filterType==='image'?'bg-yellow-500 text-black':'bg-zinc-800 text-gray-400'}`}>IMG</a>
            </div>
        </div>
        
        <div class="grid grid-cols-1 gap-3">
            {files.map(f => (
                <div class="bg-zinc-900/50 p-3 rounded-xl border border-transparent hover:border-zinc-700 flex items-center justify-between group">
                    <div class="flex items-center gap-3 overflow-hidden">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="flex gap-2 text-[10px] text-gray-500 mt-1"><span>{f.size}</span><span>{formatDate(f.uploadedAt)}</span><span>S{f.server}</span></div>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        <button onclick={`navigator.clipboard.writeText(window.location.origin + '/d/${f.server}/${f.r2Key}'); showToast('Copied!')`} class="w-8 h-8 bg-zinc-800 rounded-lg hover:bg-white hover:text-black text-gray-400"><i class="fa-regular fa-copy"></i></button>
                        <a href={`/d/${f.server}/${f.r2Key}`} target="_blank" class="w-8 h-8 flex items-center justify-center bg-zinc-800 rounded-lg hover:bg-green-600 text-white"><i class="fa-solid fa-download"></i></a>
                        <button onclick={`openDeleteModal('${f.id}')`} class="w-8 h-8 bg-zinc-800 rounded-lg hover:bg-red-600 text-white"><i class="fa-solid fa-trash"></i></button>
                    </div>
                </div>
            ))}
            {files.length===0 && <p class="text-center text-zinc-600 text-sm py-10">No files found.</p>}
        </div>
    </Layout>);
});
...
// Background Worker Logic (Simulated with async function)
async function processRemoteUpload(jobId: string, user: User, url: string, customName: string, server: string, expiry: string) {
    ...
    try {
        ...
        if(totalSize > MAX_REMOTE_SIZE) throw new Error("File too large (>2GB)");
        if(user.usedStorage + (server === 'both' ? totalSize*2 : totalSize) > PLANS[user.plan].limit) throw new Error("Storage Full");

        // 2. Validate Type & Name
        ...
        const doUpload = async (svr: "1" | "2", stream: ReadableStream) => {
            ...
            const r2Key = `${user.username}/${fileName.split('.')[0]}-${Date.now()}.${ext}`;
            ...
            upload.on("httpUploadProgress", async (p) => {
                if(totalSize) {
                    ...
                    // Only update KV if pct changed significantly to save writes
                    if(pct % 5 === 0) {
        ...
        setTimeout(() => kv.delete(jobKey), 30000); // Keep error longer
    }
}
...
app.post("/api/upload/remote", async (c) => {
    ...
    if (!isValidRemoteUrl(url)) return c.json({ error: "Invalid/Unsafe URL" }, 400);

    const jobId = crypto.randomUUID();
    const jobData: JobData = { id: jobId, username: session.user.username, name: "Pending...", status: "pending", progress: 0, totalSize: "Unknown", createdAt: Date.now() };
...
app.post("/api/upload/presign", async (c) => {
    ...
    if (session.user.usedStorage + size > limit) return c.json({ error: "Storage Full" }, 400);
    ...
    const r2Key = `${session.user.username}/${safeName.split('.')[0]}-${Date.now()}.${nCheck.ext}`;
...
app.post("/api/upload/complete", async (c) => {
    ...
    try {
        ...
        const name = key.split('/').pop().replace(/-\d+(\.[a-z0-9]+)?$/, "$1");
...
// Auth & Admin Routes (Condensed)
app.get("/login", (c) => c.html(<Layout hideLoginLink={true}><div class="max-w-sm mx-auto mt-24 glass p-8 rounded-2xl"><h1 class="text-2xl font-black text-center text-yellow-500 mb-6">LOGIN</h1><form action="/login" method="post" class="space-y-4"><input name="username" placeholder="Username" required class="w-full bg-black border border-zinc-700 p-3 rounded-xl text-white"/><input type="password" name="password" placeholder="Password" required class="w-full bg-black border border-zinc-700 p-3 rounded-xl text-white"/><button class="w-full bg-yellow-500 font-bold py-3 rounded-xl">Login</button></form><div class="text-center mt-4"><a href="/register" class="text-xs text-gray-500 hover:text-white">Create Account</a></div></div></Layout>));
...
app.get("/register", (c) => c.html(<Layout hideLoginLink={true}><div class="max-w-sm mx-auto mt-24 glass p-8 rounded-2xl"><h1 class="text-2xl font-black text-center text-white mb-6">REGISTER</h1><form action="/register" method="post" class="space-y-4"><input name="username" placeholder="Username" required class="w-full bg-black border border-zinc-700 p-3 rounded-xl text-white"/><input type="password" name="password" placeholder="Password (Min 6)" required class="w-full bg-black border border-zinc-700 p-3 rounded-xl text-white"/><button class="w-full bg-green-600 font-bold py-3 rounded-xl text-white">Sign Up</button></form></div></Layout>));
