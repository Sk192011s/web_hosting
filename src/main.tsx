                    ...
                    {props.user ? (
                        <div class="flex gap-3 items-center">
                            <div class="hidden sm:flex flex-col items-end leading-tight">
                                ...
                                {checkVipStatus(props.user) ? <span class="text-[9px] text-yellow-500 font-bold">VIP</span> : <span class="text-[9px] text-gray-500 font-bold">FREE</span>}
                            </div>
                            {props.user.username === ADMIN_USERNAME && <a href="/admin" class="w-8 h-8 flex items-center justify-center bg-purple-600 rounded-full"><i class="fa-solid fa-shield-halved text-xs"></i></a>}
                            <a href="/logout" class="w-8 h-8 flex items-center justify-center bg-zinc-800 border border-zinc-700 rounded-full hover:bg-red-600/20 hover:text-red-500"><i class="fa-solid fa-power-off text-xs"></i></a>
                        </div>
                    ) : (
        ...
        <Layout user={user}>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div class="glass p-5 rounded-2xl relative overflow-hidden group">
                    ...
                    <p class={`text-2xl font-black ${isVip ? 'text-yellow-500' : 'text-zinc-300'}`}>{isVip ? "VIP PRO" : "Free Plan"}</p>
                    {isVip && user.vipExpiry && <p class="text-[10px] text-green-400 mt-2 font-mono bg-green-900/20 inline-block px-2 py-1 rounded">EXP: {formatDate(user.vipExpiry)}</p>}
                ...
                <div class="glass p-5 rounded-2xl relative">
                    <div class="flex justify-between items-end mb-2">
                        <div><p class="text-xs text-zinc-400 uppercase font-bold">Storage</p><p class="text-xl font-bold text-white">{totalMB} <span class="text-sm text-zinc-500">/ {displayLimit}</span></p></div>
                        <span class="text-2xl font-black text-zinc-600">{usedPercent.toFixed(0)}%</span>
                    </div>
                    <div class="w-full bg-zinc-800 rounded-full h-3 overflow-hidden"><div class={`h-full rounded-full ${isVip ? 'bg-gradient-to-r from-yellow-600 to-yellow-400' : 'bg-zinc-500'}`} style={`width: ${usedPercent}%`}></div></div>
                </div>
                <div class="glass p-5 rounded-2xl flex items-center justify-between">
                    <div><p class="text-xs text-zinc-400 uppercase font-bold">ဖိုင်များ</p><p class="text-3xl font-black text-white">{files.length}</p></div>
                    <div class="w-12 h-12 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-500 text-2xl"><i class="fa-solid fa-folder-open"></i></div>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            {isVip ? (
                                ...
                                <div class="relative"><input disabled value="1 Month (Free Limit)" class="w-full bg-zinc-900 border border-zinc-700 rounded-xl p-3 text-sm text-zinc-500 cursor-not-allowed" /><input type="hidden" name="expiry" value="30" /><i class="fa-solid fa-lock absolute right-4 top-3.5 text-zinc-600"></i></div>
                            )}
                        </div>
                    </div>
            ...
            <div class="glass rounded-2xl overflow-hidden border border-zinc-700/50">
                <div class="max-h-[500px] overflow-y-auto custom-scroll p-2 space-y-2">
                    ...
                    {files.length === 0 && <div class="text-center text-zinc-500 py-12 flex flex-col items-center"><i class="fa-solid fa-folder-open text-4xl mb-3 opacity-20"></i><p class="text-sm">ဖိုင်များ မရှိသေးပါ။</p></div>}
                </div>
            </div>
        </Layout>
    );
...
app.post("/api/get-upload-url", async (c) => {
    ...
    const command = new PutObjectCommand({
        ...
        Key: r2Key,
        ContentType: type,
        ContentDisposition: 'inline' // 🔥 Fix for Auto-Download (Allows playback)
    });
...
app.get("/admin", async (c) => {
    ...
    return c.html(<Layout title="Admin Panel" user={{ username: ADMIN_USERNAME, isVip: true } as any}><h1 class="text-2xl font-bold mb-6">User Management</h1><div class="space-y-4">{users.map(u => (<div class="bg-zinc-800/50 p-4 rounded-xl border border-zinc-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><p class="font-bold text-white flex items-center gap-2">{u.username} {checkVipStatus(u) ? <span class="bg-yellow-500 text-black px-1.5 rounded text-[10px] font-bold">VIP</span> : <span class="bg-zinc-600 px-1.5 rounded text-[10px]">FREE</span>}</p><p class="text-xs text-gray-400 mt-1">Used: {(u.usedStorage/1024/1024).toFixed(2)} MB</p>{u.vipExpiry && <p class="text-[10px] text-green-400 mt-0.5">Exp: {formatDate(u.vipExpiry)}</p>}</div><form action="/admin/add-vip" method="post" class="flex gap-2"><input type="hidden" name="username" value={u.username} /><select name="days" class="bg-zinc-900 text-xs p-2 rounded-lg border border-zinc-600 outline-none focus:border-purple-500"><option value="30">Add 1 Month</option><option value="90">Add 3 Months</option><option value="365">Add 1 Year</option><option value="-1">Remove VIP</option></select><button class="bg-purple-600 hover:bg-purple-500 px-4 py-2 rounded-lg text-xs font-bold transition shadow-lg">Save</button></form></div>))}</div></Layout>);
});
app.post("/admin/add-vip", async (c) => {
    ...
    if(user) {
        ...
        else { user.isVip = true; const now = Date.now(); const currentExp = (user.vipExpiry && user.vipExpiry > now) ? user.vipExpiry : now; user.vipExpiry = currentExp + (addDays * 24 * 60 * 60 * 1000); }
        await kv.set(["users", user.username], user);
    }
